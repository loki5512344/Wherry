...
pub enum SortCol { Name, Size, Kind, Modified }
    ...
    F: Fn(&FileEntry) -> Option<DragPayload>,
{
    ...
    let (sort_col, sort_dir) = ui.ctx().data_mut(|d| {
        *d.get_temp_mut_or(sort_id, (SortCol::Name, SortDir::Asc))
    });
    ...
    header_frame.show(ui, |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                let mod_r = sort_header(ui, "Modified", 140.0, sort_col == SortCol::Modified, sort_dir);
                if mod_r { new_sort = Some(toggle_sort(SortCol::Modified, sort_col, sort_dir)); }

                let type_r = sort_header(ui, "Type", 40.0, sort_col == SortCol::Kind, sort_dir);
                if type_r { new_sort = Some(toggle_sort(SortCol::Kind, sort_col, sort_dir)); }

                let size_r = sort_header(ui, "Size", 70.0, sort_col == SortCol::Size, sort_dir);
                if size_r { new_sort = Some(toggle_sort(SortCol::Size, sort_col, sort_dir)); }
            });
        });
    });

    if let Some(s) = new_sort {
        ui.ctx().data_mut(|d| { d.insert_temp(sort_id, s); });
    }
                        ...
                        ui.horizontal_centered(|ui| {
                            ...
                            if let Some(payload) = make_drag_payload(entry) {
                                ui.dnd_drag_source(row_id, payload, |ui| {
                                    ui.label(
                                        RichText::new(&name_text).color(name_col).size(12.0)
                                    );
                                });
                            } else {
    ...
    FileTableResponse { double_clicked, clicked }
}
...
fn toggle_sort(col: SortCol, cur_col: SortCol, cur_dir: SortDir) -> (SortCol, SortDir) {
    if col == cur_col {
        let new_dir = if cur_dir == SortDir::Asc { SortDir::Desc } else { SortDir::Asc };
        (col, new_dir)
    } else {
        (col, SortDir::Asc)
    }
